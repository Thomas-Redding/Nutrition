...
def claimIndex():
	path = "../Database-Files/Meta-Files/user-food-index-count.txt"
	f = open(path, "r+")
	rtn = int(f.read())
	f.seek(0)
	f.truncate()
...
def createNewFood(fileName, contents):
	f = open("../Database-Files/User-Foods/" + str(fileName) + ".txt", "w+")
	f.write(contents)
	f.close()
...
def addFoodToTricharIndex(food_id, food_name):
...
def addFoodToNameIndex(food_id, food_name):
	path = "../Database-Files/Index-To-Name/" + str(food_id) + ".txt"
	f = open(path, "w+")
	f.write(food_name)
	f.close()
...
if "nutrients" not in data:
	print "Error: JSON doen't contain \"nutrients\""
	sys.exit(0)


index = claimIndex()
addFoodToTricharIndex(index, data["short_name"])
addFoodToNameIndex(index, data["short_name"])
